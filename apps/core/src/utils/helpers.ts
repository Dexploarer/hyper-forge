...
export function generateMaterialDescription(
  ...
  const _adjective =
    tier.adjectives[Math.floor(Math.random() * tier.adjectives.length)];
...
export function generateTierBatch(
  ...
  return materialTiers.map((tier) => {
    ...
    return {
      ...
      description: generateMaterialDescription(
...
/**
 * Format date as local date string (e.g., "Jan 15, 2025")
 */
...
/**
 * Format date with time (e.g., "Jan 15 at 3:45 PM")
 */
