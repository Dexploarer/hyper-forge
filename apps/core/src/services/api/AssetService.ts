...
class AssetServiceClass {
  ...
  async bulkUpdateAssets(
    ...
    // Type assertion needed because Eden Treaty doesn't support dynamic route names with hyphens
    const bulkUpdateEndpoint = (api.api.assets as any)["bulk-update"];
    const { data, error } = await bulkUpdateEndpoint.post({
      assetIds,
      updates,
    });
