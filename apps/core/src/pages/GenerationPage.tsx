...
export const GenerationPage: React.FC<GenerationPageProps> = ({
  ...
  // Load prompts
  const { data: gameStylePrompts, isLoading: gameStyleLoading } =
    useGameStylePrompts();
  const saveCustomGameStyleMutation = useSaveGameStylePromptsMutation();
  const deleteCustomGameStyleMutation = useDeleteGameStyleMutation();

  const { data: loadedAssetTypePrompts, isLoading: _assetTypeLoading } =
    useAssetTypePrompts();
  const saveCustomAssetTypeMutation = useSaveAssetTypePromptsMutation();
  const deleteCustomAssetTypeMutation = useDeleteAssetTypeMutation();

  const { data: materialPromptTemplates } = useMaterialPromptTemplates();

  // Helper functions for mutations
  const getTypesByGeneration = useCallback(
    (generationType: "avatar" | "item") => {
      if (!loadedAssetTypePrompts) return {};
      const category = loadedAssetTypePrompts[generationType];
      if (!category) return {};
      return { ...category.default, ...category.custom };
    },
    [loadedAssetTypePrompts],
  );

  const saveCustomGameStyle = useCallback(
    async (
      styleId: string,
      styleData: { name: string; base: string },
    ): Promise<boolean> => {
      if (!gameStylePrompts) return false;
      ...
      try {
        await saveCustomGameStyleMutation.mutateAsync(updatedPrompts);
        return true;
      } catch (error) {
        console.error("Failed to save custom game style:", error);
        return false;
      }
    },
    [gameStylePrompts, saveCustomGameStyleMutation],
  );

  const deleteCustomGameStyle = useCallback(
    async (styleId: string): Promise<boolean> => {
      try {
        await deleteCustomGameStyleMutation.mutateAsync(styleId);
        return true;
      } catch (error) {
        console.error("Failed to delete custom game style:", error);
        return false;
      }
    },
    [deleteCustomGameStyleMutation],
  );

  const saveCustomAssetType = useCallback(
    (
      typeId: string,
      typeData: { name: string; prompt: string; placeholder?: string },
      ...
      const updatedPrompts = {
        ...loadedAssetTypePrompts,
        [category]: {
          ...loadedAssetTypePrompts[category],
          custom: {
            ...loadedAssetTypePrompts[category].custom,
            [typeId]: typeData,
          },
        },
      };
      saveCustomAssetTypeMutation.mutate(updatedPrompts);
    },
    [loadedAssetTypePrompts, saveCustomAssetTypeMutation],
  );

  const deleteCustomAssetType = useCallback(
    async (typeId: string, category: "avatar" | "item"): Promise<boolean> => {
      try {
        await deleteCustomAssetTypeMutation.mutateAsync({ typeId, category });
        return true;
      } catch (error) {
        console.error("Failed to delete custom asset type:", error);
        return false;
      }
    },
    [deleteCustomAssetTypeMutation],
  );
  ...
  const currentGenerationTypes = useMemo(() => {
    ...
    // Support all generation types: avatar, item, building, environment, prop
    if (generationType === "avatar" || generationType === "item") {
      return getTypesByGeneration(generationType);
    }
    // For building/environment/prop, return empty for now (will be populated as needed)
    return {};
    ...
    if (shouldGenerateWorld && !isGenerating) {
      ...
      const timer = setTimeout(async () => {
        try {
          ...
          const { data, error } = await (api.api.content as any)[
            "generate-world"
          ].post({
            theme: "fantasy",
            complexity: "medium",
  ...
  const handleSaveCustomAssetTypes = async () => {
    ...
    // Currently only 'avatar' and 'item' have backend support for custom types
    // For building/environment/prop, this will be added when backend support is ready
    if (generationType !== "avatar" && generationType !== "item") {
      notify.info(
        "Custom asset type saving for this generation type is coming soon!",
      );
      return;
    }
  ...
  const handleStartGeneration = async () => {
    ...
    const config = buildGenerationConfig({
      ...
      materialPromptTemplates:
        materialPromptTemplates?.templates &&
        "runescape" in materialPromptTemplates.templates &&
        "generic" in materialPromptTemplates.templates
          ? (materialPromptTemplates.templates as {
              runescape: string;
              generic: string;
            })
          : undefined,
