...
export function MeshFittingDebugger({ onClose }: MeshFittingDebuggerProps) {
  ...
  const { data: assets, isLoading: _loading } = useAssets();

  // Transform assets into the format expected by the component
  const availableAvatars = React.useMemo(() => {
    return (assets || [])
      .filter(
        (asset) => asset.type === "character" && asset.hasModel && asset.cdnUrl,
      )
      .map((asset) => ({
        id: asset.id,
        name: asset.name,
  ...
  const [, /* skinnedArmorMesh */ setSkinnedArmorMesh] =
  ...
  const handleViewModeChange = (
    newMode: "sphereCube" | "avatarArmor" | "helmetFitting",
  ) => {
    ...
    if (sceneRef.current) {
      ...
      scene.traverse((obj) => {
        ...
        if (viewMode === "helmetFitting" && newMode !== "helmetFitting") {
          if (
            obj.userData?.isHelmet ||
            obj.name === "HelmetClone" ||
                  ...
                  <div className="flex items-center gap-3 mb-6">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-text-secondary mt-0.5">
                        {viewMode === "avatarArmor"
                          ? "Choose avatar and armor"
                          : "Choose avatar and helmet"}
                      </p>
                    </div>
                  </div>
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-text-tertiary mt-1">
                        {selectedAvatar?.name || "No avatar selected"} character
                        model
                      </p>
                    </div>
                      ...
                      <div>
                        ...
                        <select
                          ...
                          onChange={(
                            ...
                            const armor = availableArmors.find(
                              (a) => a.id === e.target.value,
                            );
                            if (armor) {
                              setSelectedArmor({
                        ...
                        <p className="text-xs text-text-tertiary mt-1">
                          {selectedArmor?.name || "No armor selected"} armor
                          variant
                        </p>
                      </div>
                    ) : viewMode === "helmetFitting" ? (
                      <div>
                        ...
                        <select
                          ...
                          onChange={(
                            ...
                            const helmet = availableHelmets.find(
                        ...
                        <p className="text-xs text-text-tertiary mt-1">
                          {selectedHelmet?.name || "No helmet selected"}
                        </p>
                      </div>
                    ) : null}
                  </div>
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-xs text-text-secondary">
                        {fittingParameters.stepSize?.toFixed(2) || 0.1}
                      </span>
                    </div>
                    <RangeInput
                      min={0}
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-xs text-text-secondary">
                        {fittingParameters.smoothingRadius?.toFixed(1) || 2}
                      </span>
                    </div>
                    <RangeInput
                      min={0}
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-xs text-text-secondary">
                        {fittingParameters.smoothingStrength.toFixed(2)}
                      </span>
                    </div>
                    <RangeInput
                      min={0}
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-xs text-text-secondary">
                        {(fittingParameters.targetOffset * 100).toFixed(1)}cm
                      </span>
                    </div>
                    <RangeInput
                      min={0}
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-xs text-text-secondary">
                        {((fittingParameters.sampleRate || 0.5) * 100).toFixed(
                          0,
                        )}
                        %
                      </span>
                    ...
                    {viewMode === "avatarArmor" && (
                      <>
                        <Checkbox
                          ...
                          onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                            updateFittingParameters({
                              preserveOpenings: e.target.checked,
                            })
                          }
                        ...
                        <Checkbox
                          checked={
                            fittingParameters.pushInteriorVertices || false
                          }
                          onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                            updateFittingParameters({
                  ...
                  {viewMode === "avatarArmor" && (
                    <div className="space-y-2 border-t border-white/10 pt-4 mt-4">
                      ...
                      <Checkbox
                        ...
                        onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                          updateFittingParameters({
                            showDebugArrows: e.target.checked,
                          })
                        }
                        ...
                        <div className="ml-6 space-y-2">
                          <div>
                            <div className="flex items-center justify-between mb-1">
                              ...
                              <span className="text-xs text-text-secondary">
                                1/{fittingParameters.debugArrowDensity || 10}
                              </span>
                            </div>
                            <RangeInput
                              min={1}
                              ...
                              onChange={(
                                ...
                                updateFittingParameters({
                                  debugArrowDensity: parseInt(e.target.value),
                                })
                              }
                            />
                          ...
                          <div>
                            ...
                            <select
                              ...
                              onChange={(
                                ...
                                updateFittingParameters({
                                  debugColorMode: e.target.value as
                                    | "direction"
                                    | "magnitude"
                                    | "sidedness",
                                })
                          ...
                          <div className="mt-3 p-2 bg-bg-secondary/50 rounded">
                            ...
                            {(fittingParameters.debugColorMode ||
                              "direction") === "direction" ? (
                              <div className="space-y-1 text-xs">
                                <div className="flex items-center gap-2">
                                  <div className="w-3 h-3 bg-red-500 rounded-sm"></div>
                                  <span className="text-text-tertiary">
                                    Forward (bad for back vertices)
                                  </span>
                                </div>
                                <div className="flex items-center gap-2">
                                  <div className="w-3 h-3 bg-green-500 rounded-sm"></div>
                                  <span className="text-text-tertiary">
                                    Backward (good for back vertices)
                                  </span>
                                </div>
                                <div className="flex items-center gap-2">
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      ...
                      <RangeInput
                        ...
                        onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                          setHelmetSizeMultiplier(parseFloat(e.target.value))
                        }
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <RangeInput
                        ...
                        onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                          setHelmetFitTightness(parseFloat(e.target.value))
                        }
                      />
                      <p className="text-xs text-text-tertiary mt-1">
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className="text-xs text-text-secondary">
                          {helmetVerticalOffset.toFixed(2)}
                        </span>
                      </div>
                      <RangeInput
                        min={-0.1}
                        ...
                        onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                          setHelmetVerticalOffset(parseFloat(e.target.value))
                        }
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <RangeInput
                        ...
                        onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                          setHelmetForwardOffset(parseFloat(e.target.value))
                        }
                      />
                    </div>
                    ...
                    <div className="pt-2 space-y-2 border-t border-white/10 mt-4">
                      ...
                      <Checkbox
                        ...
                        onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                          setShowHeadBounds(e.target.checked)
                        }
            ...
            <div className="absolute inset-0">
              ...
              >
                <Scene
                  ...
                  viewMode={viewMode}
                  selectedAvatarPath={
                    selectedAvatar?.path || selectedAvatarPath
                  }
                  selectedArmorPath={selectedArmor?.path || selectedArmorPath}
                  selectedHelmetPath={
                    selectedHelmet?.path || selectedHelmetPath
                  }
                        ...
                        <>
                          ...
                          <div className="relative debug-dropdown-container">
                            ...
                            >
                              ...
                              <span>
                                {showDebugOptions ? "Hide" : "Show"} Debug
                                Options
                              </span>
                              <ChevronDown
                                className={cn(
                                    ...
                                    onClick={async () => {
                                      ...
                                      try {
                                        ...
                                        BoneDiagnostics.analyzeSkeletonForExport(
                                          ...
                                          "Bound Armor Skeleton",
                                        );

                                        // Test with different scales
                                        const testSkeletonM =
                                          BoneDiagnostics.createTestSkeleton(
                                            "meters",
                                          );
                                        const testSkeletonCM =
                                          BoneDiagnostics.createTestSkeleton(
                                            "centimeters",
                                          );
                                        ...
                                        console.log(
                                          "Diagnostics complete - check console",
                                        );
                                      } catch (error) {
                                        console.error(
                                          "Diagnostics failed:",
