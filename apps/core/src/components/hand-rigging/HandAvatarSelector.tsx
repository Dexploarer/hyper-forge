...
export function HandAvatarSelector() {
  ...
  const avatarAssets = (assets || []).filter(
    (a: Asset) => a.type === "character",
  );
  const filteredAvatars = avatarAssets.filter((a: Asset) =>
  ...
  const handleSelectAvatar = (avatar: Asset) => {
    ...
    // Check if avatar has CDN URL
    if (!avatar.cdnUrl) {
      setError(`Avatar ${avatar.id} does not have a CDN URL`);
      return;
    }
    ...
    if (animations?.tpose && avatar.cdnFiles && avatar.cdnFiles.length > 0) {
      ...
      const tposeFile = avatar.cdnFiles.find((file) =>
      ...
      if (tposeFile) {
        ...
        console.log(`T-pose file not found in CDN files, using base model`);
      }
    } else {
                ...
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      {hasAnimations(avatar) &&
                        avatar.metadata.animations?.basic?.tpose && (
                          <Badge
                            variant="success"
                            size="sm"
            ...
            <div className="flex items-center gap-2 text-sm">
              ...
              <span className="text-text-primary">
                ...
                {hasAnimations(selectedAvatar) &&
                  selectedAvatar.metadata.animations?.basic?.tpose && (
                    <Badge
                      variant="success"
                      size="sm"
