...
const RetextureModal: React.FC<RetextureModalProps> = ({
  ...
  const { data: presets, isLoading: _presetsLoading } = useMaterialPresets();
  const { mutateAsync: retextureAsset, isPending: isRetexturing } =
    useRetexturing();
          ...
          onClick={async () => {
            ...
            try {
              if (workflowMode === "presets") {
                ...
                setResults([
                  { name: "Image Reference Texture", status: "pending" },
                ]);
                setProgress(0);
                ...
                if (!imageUrl && uploadedImage) {
                  ...
                  console.warn(
                    "File upload not yet implemented. Please use an image URL instead.",
                  );
                  setResults([
                    {
