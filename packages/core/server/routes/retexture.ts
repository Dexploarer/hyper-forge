  ...
  return new Elysia({ prefix: '/api', name: 'retexture' })
    ...
    .post('/retexture', async ({ body }) => {
      ...
      detail: {
        ...
        description: 'Creates a new material variant of an existing asset using Meshy AI. (Auth optional - authenticated users get ownership tracking)'
      }
    })

    // Regenerate base model endpoint
    .post('/regenerate-base/:baseAssetId', async ({ params: { baseAssetId } }) => {
      ...
      detail: {
        ...
        description: 'Regenerates the base 3D model using Meshy AI. (Auth optional - authenticated users get ownership tracking)'
      }
    })
}
