    ...
    .guard({
      ...
      .post('/pipeline', async ({ body }) => {
        ...
        detail: {
          ...
          description: 'Initiates a new AI-powered 3D asset generation pipeline. (Auth optional - authenticated users get ownership tracking)'
        }
      })

      // Get pipeline status
      .get('/pipeline/:pipelineId', async ({ params: { pipelineId } }) => {
        ...
        detail: {
          ...
          description: 'Returns the current status and progress of a generation pipeline. (Auth optional)'
        }
      })
    )
}
