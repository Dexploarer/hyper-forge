...
export async function loadPromptFile(promptType: PromptType): Promise<PromptData | null> {
  // Check cache first
...
export async function savePromptFile(promptType: PromptType, data: PromptData): Promise<boolean> {
  const filePath = path.join(__dirname, '../../public/prompts', `${promptType}.json`)
...
export async function loadAllPrompts(): Promise<AllPrompts> {
  ...
  for (const type of promptTypes) {
    ...
    if (data) {
      ...
      const key = type.replace(/-prompts$/, '').replace(/-(.)/g, (_, char) => char.toUpperCase()) as keyof AllPrompts
      prompts[key] = data as any
    }
  }
...
export async function getGameStylePrompt(style: string = 'generic'): Promise<string> {
  const prompts = await loadPromptFile('game-style-prompts') as GameStylePrompts | null
  if (!prompts) return 'game-ready' // Fallback

  // Check custom first, then default
  const customPrompt = prompts.custom?.[style]
  if (customPrompt) return customPrompt.base || customPrompt.enhanced || 'game-ready'

  const defaultPrompt = prompts.default?.[style]
  if (defaultPrompt) return defaultPrompt.base || defaultPrompt.enhanced || 'game-ready'
...
export async function getAssetTypePrompt(assetType: string): Promise<string> {
  const prompts = await loadPromptFile('asset-type-prompts') as AssetTypePrompts | null
...
export async function getMaterialPromptTemplate(gameStyle: string = 'generic'): Promise<string> {
  const prompts = await loadPromptFile('material-prompts') as MaterialPrompts | null
  if (!prompts) {
    // Fallback templates
...
export async function getGenerationPrompts(): Promise<GenerationPrompts> {
  const prompts = await loadPromptFile('generation-prompts') as GenerationPrompts | null
  if (!prompts) {
    // Fallback prompts
    return {
      version: '1.0.0',
...
export async function getGPT4EnhancementPrompts(): Promise<GPT4EnhancementPrompts | Record<string, never>> {
  const prompts = await loadPromptFile('gpt4-enhancement-prompts') as GPT4EnhancementPrompts | null
  return prompts || {} // Return empty object if not found
}

export async function getWeaponDetectionPrompts(): Promise<WeaponDetectionPrompts | Record<string, never>> {
  const prompts = await loadPromptFile('weapon-detection-prompts') as WeaponDetectionPrompts | null
  return prompts || {} // Return empty object if not found
}
