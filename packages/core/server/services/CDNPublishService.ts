...
export class CDNPublishService {
  ...
  async publishAsset(assetId: string): Promise<PublishResult> {
    try {
      ...
      if (riggedModelCdnUrl) {
        logger.info(
          { context: "CDNPublish" },
          `   Rigged Model URL: ${riggedModelCdnUrl}`,
        );
      }
      logger.info(
        { context: "CDNPublish" },
        `   All URLs: ${cdnUrls.join(", ")}`,
      );
      ...
      logger.error({ err: error }, `Failed to publish ${assetId} to CDN:`);
      return {
        success: false,
        assetId,
        filesPublished: [],
  ...
  async updateAssetCDNUrls(
    ...
    try {
      ...
      logger.info(
        { context: "CDNPublish" },
        `✅ Updated database with CDN URLs for ${assetId}`,
      );
    } catch (error) {
      logger.error(
        { err: error },
        `❌ Failed to update database for ${assetId}:`,
      );
      throw error;
    }
  }
