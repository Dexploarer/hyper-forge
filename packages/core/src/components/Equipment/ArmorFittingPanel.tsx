import {
  Target, Download, Save, Eye, EyeOff, Link, Play, Pause,
  Layers, Activity, RefreshCw
} from 'lucide-react'
import React from 'react'

import { ArmorFittingViewerRef } from '../ArmorFitting/ArmorFittingViewer'
...
interface ArmorFittingPanelProps {
  ...
  onVisualizationModeChange: (mode: 'none' | 'regions' | 'collisions' | 'weights' | 'hull') => void
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="space-y-3">
          ...
          >
            {isFitting ? (
              ...
              <>
                ...
                <span>{isHelmetMode ? 'Auto-Fit Helmet' : 'Auto-Fit Armor'}</span>
              </>
            )}
          </button>
          ...
          {!isHelmetMode && (
            <div className="flex items-center justify-between p-3 rounded-lg bg-bg-secondary border border-border-primary">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium text-text-primary">Weight Transfer</span>
              </div>
          ...
          {!isHelmetMode && isArmorFitted && !isArmorBound && onBindArmorToSkeleton && (
            <button
          ...
          {isHelmetMode && isHelmetFitted && !isHelmetAttached && onAttachHelmetToHead && (
            <button
        ...
        <div className="space-y-2">
          <label className="text-sm font-medium text-text-secondary">Visualization</label>
          <select
        ...
        <button
          ...
          className={cn(
            ...
            (!isArmorFitted && !isHelmetFitted) && "opacity-50 cursor-not-allowed"
          )}
        >
          {isExporting ? (
            <>
      ...
      <div className="space-y-3">
        ...
        {isHelmetMode ? (
          ...
          <>
            <div>
              <div className="flex items-center justify-between mb-1">
                <label className="text-xs font-medium text-text-secondary">Size Multiplier</label>
                <span className="text-xs text-text-secondary">{helmetSizeMultiplier.toFixed(2)}</span>
              </div>
              <RangeInput
                min="0.8"
                max="1.2"
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                <label className="text-xs font-medium text-text-secondary">Vertical Offset</label>
                <span className="text-xs text-text-secondary">{helmetVerticalOffset.toFixed(3)}m</span>
              </div>
              <RangeInput
                min="-0.2"
                max="0.2"
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                <label className="text-xs font-medium text-text-secondary">Forward Offset</label>
                <span className="text-xs text-text-secondary">{helmetForwardOffset.toFixed(3)}m</span>
              </div>
              <RangeInput
                min="-0.2"
                max="0.2"
            ...
            {/* Rotation */}
            <div className="space-y-2">
              <label className="text-xs font-medium text-text-secondary">Rotation</label>
              {(['x', 'y', 'z'] as const).map((axis) => (
                <div key={axis}>
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-xs text-text-secondary uppercase">{axis}</span>
                    <span className="text-xs text-text-secondary">{helmetRotation[axis].toFixed(0)}°</span>
                  </div>
                  <RangeInput
                    min="-180"
                    max="180"
          ...
          <>
            <div>
              <div className="flex items-center justify-between mb-1">
                <label className="text-xs font-medium text-text-secondary">Iterations</label>
                <span className="text-xs text-text-secondary">{fittingConfig.iterations}</span>
              </div>
              <RangeInput
                min="1"
                max="15"
                ...
                onChange={(e) => onFittingConfigChange({ iterations: parseInt(e.target.value) })}
              />
            </div>

            <div>
              <div className="flex items-center justify-between mb-1">
                <label className="text-xs font-medium text-text-secondary">Step Size</label>
                <span className="text-xs text-text-secondary">{(fittingConfig.stepSize ?? 0.15).toFixed(2)}</span>
              </div>
              <RangeInput
                min="0.05"
                max="0.3"
                ...
                onChange={(e) => onFittingConfigChange({ stepSize: parseFloat(e.target.value) })}
              />
            </div>

            <div>
              <div className="flex items-center justify-between mb-1">
                <label className="text-xs font-medium text-text-secondary">Target Offset</label>
                <span className="text-xs text-text-secondary">{(fittingConfig.targetOffset ?? 0.05).toFixed(2)}</span>
              </div>
              <RangeInput
                min="0.01"
                max="0.15"
                ...
                onChange={(e) => onFittingConfigChange({ targetOffset: parseFloat(e.target.value) })}
              />
            </div>
          </>
        )}
