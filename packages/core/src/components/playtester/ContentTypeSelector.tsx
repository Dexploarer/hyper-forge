    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="text-center mb-8">
        ...
        <p className="text-text-secondary">
          Deploy multiple AI agents to test your game content for bugs, balance,
          and engagement
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-5xl mx-auto">
        {CONTENT_TYPES.map((contentType) => {
          ...
          // Extract gradient colors for inline styles to ensure proper rendering
          const getGradientColors = (
            colorClass: string,
          ): { from: string; to: string } => {
            const match = colorClass.match(/from-(\S+)\s+to-(\S+)/);
            if (!match) return { from: "#8B5CF6", to: "#8B5CF6" }; // fallback to purple
            ...
            return {
              from: colorMap[match[1]] || "#8B5CF6",
              to: colorMap[match[2]] || "#8B5CF6",
            };
          };

          const gradientColors = getGradientColors(contentType.color);
      ...
      <div className="mt-12 max-w-3xl mx-auto p-6 bg-bg-secondary/50 border border-border-primary rounded-xl">
        ...
        <div className="space-y-2 text-sm text-text-secondary">
          ...
          <p>
            • AI agents test in parallel, finding bugs and rating engagement
          </p>
