...
interface AnimationPlayerProps {
  cdnUrl: string; // Not used anymore, kept for compatibility
  animations?: {
    basic?: {
      walking?: string;
      running?: string;
  ...
  assetId?: string; // Optional asset ID if not extractable from cdnUrl
}
...
export const AnimationPlayer: React.FC<AnimationPlayerProps> = ({
  ...
  // Extract asset ID from the CDN URL or use provided assetId
  const extractedAssetId =
    cdnUrl?.match(new RegExp("^/api/assets/([^/]+)/model"))?.[1] || "";
  ...
  const handlePlayAnimation = async (
    animId: "resting" | "tpose" | "walking" | "running",
  ) => {
    ...
    if (animations?.basic && assetId) {
      ...
      if (newModelUrl && newModelUrl !== primaryModelUrl) {
        ...
        console.log(
          `Switching to model: ${newModelUrl} for animation: ${animId}`,
        );
  ...
  if (!primaryModelUrl) {
    return (
      <div
        className={`relative ${className} flex items-center justify-center bg-gradient-to-br from-bg-primary to-bg-secondary`}
      >
        <div className="text-center p-8">
          <Activity className="w-12 h-12 text-text-muted mb-4 mx-auto" />
          <p className="text-text-tertiary">
            No animations available for this asset
          </p>
        </div>
      </div>
    );
            ...
            {availableAnimations.map((anim) => (
              <Button
                ...
                variant={
                  ...
                  (anim.id === "resting" || anim.id === "tpose" || isPlaying)
                    ? "primary"
                    : "secondary"
                }
                onClick={() =>
                  handlePlayAnimation(
                    anim.id as "resting" | "tpose" | "walking" | "running",
                  )
                }
                className="text-xs"
            ...
            {bonesData.length > 0 ? (
              <div className="font-mono text-xs space-y-1">
                {bonesData.map((bone, index) => (
                  <div
                    key={index}
                    className="flex gap-2 p-2 bg-bg-secondary rounded"
                  >
                    <span className="text-text-muted min-w-[3rem]">
                      [{index}]
                    </span>
