...
function layoutVariantTree(
  ...
  baseAssets.forEach((baseAsset, baseIndex) => {
    ...
    if (variants.length > 0) {
      const totalWidth = Math.max((variants.length - 1) * HORIZONTAL_SPACING, 0)
...
export const VariantTreeViewer: React.FC<VariantTreeViewerProps> = ({ assets, onClose }) => {
  ...
  const { baseAssets, variantMap } = useMemo(() => {
    ...
    bases.forEach(base => {
      const variants = assets.filter(a =>
        a.metadata.isVariant && a.metadata.parentBaseModel === base.id
      )
      ...
      <div className="bg-bg-primary border border-border-primary rounded-xl shadow-2xl w-full max-w-7xl h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="p-4 border-b border-border-primary flex items-center justify-between bg-gradient-to-r from-bg-secondary to-bg-primary">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-semibold text-text-primary">Variant Relationship Tree</h2>
              <p className="text-sm text-text-tertiary">
                {baseAssets.length} base model{baseAssets.length !== 1 ? 's' : ''}, {totalVariants} variant
                {totalVariants !== 1 ? 's' : ''}
              </p>
            </div>
          </div>
        ...
        <div className="flex-1 relative bg-bg-secondary">
          {baseAssets.length > 0 ? (
            ...
            >
              ...
              />
              <Controls
                className="bg-bg-primary border border-border-primary rounded-lg shadow-lg"
              />
              <MiniMap
                className="bg-bg-primary border border-border-primary rounded-lg"
                nodeColor={(node: Node) => {
                  const data = node.data as VariantTreeNodeData
                  return data.asset?.metadata.isBaseModel ? '#3b82f6' : '#8b5cf6'
                }}
              />
            </ReactFlow>
          ) : (
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-lg font-medium text-text-secondary mb-2">No Base Models Found</p>
                <p className="text-sm text-text-tertiary max-w-md">
                  Create base models and generate variants to see the relationship tree.
                </p>
              </div>
            </div>
          )}
