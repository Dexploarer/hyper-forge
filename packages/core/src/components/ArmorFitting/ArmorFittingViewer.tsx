  ...
  const animationPath = useMemo(() => {
    if (needsAnimationFile && avatarUrl) {
      ...
      if (apiMatch) {
        ...
        // Use the API endpoint to get animation files from animations subdirectory
        return `/api/assets/${assetId}/animations/${animFileName}`;
      }
      ...
      if (gddMatch) {
        ...
        const animFileName =
          currentAnimation === "walking" ? "walking.glb" : "running.glb";
        return `./gdd-assets/${characterName}/animations/${animFileName}`;
      }
    }
    return null;
    ...
    apiFetch(animationPath, { method: "HEAD" })
      .then((response) => {
        if (response.ok) {
          ...
          console.log(
            `Animation file not found (404): ${animationPath} - will use built-in animations if available`,
          );
          setAnimationGltf(null);
        }
      })
