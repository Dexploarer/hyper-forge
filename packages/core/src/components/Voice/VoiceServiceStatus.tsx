...
export const VoiceServiceStatus: React.FC<VoiceServiceStatusProps> = ({
  ...
  const fetchStatus = async () => {
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to fetch voice service status')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  if (isLoading && !status) {
    return (
      <div className={cn('bg-bg-secondary border border-border-primary rounded-xl p-6', className)}>
      ...
      <div className={cn('bg-bg-secondary border border-border-primary rounded-xl p-6', className)}>
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            <h3 className="text-base font-semibold text-red-400">Failed to Load Status</h3>
            <p className="text-sm text-text-secondary mt-1">{error || 'Unknown error occurred'}</p>
            <button
              onClick={fetchStatus}
  ...
  const { isAvailable, subscription, rateLimit, voiceCount, modelCount } = status

  // Calculate usage percentage and status
  const usagePercentage = subscription
    ? voiceStatusClient.getCharacterUsagePercentage(subscription)
    : 0
  const daysUntilReset = subscription ? voiceStatusClient.getDaysUntilReset(subscription) : 0
  ...
  return (
    <div className={cn('bg-bg-secondary border border-border-primary rounded-xl overflow-hidden', className)}>
      {/* Header */}
      <div className="px-6 py-4 bg-gradient-to-r from-primary/10 to-accent/10 border-b border-border-primary">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-bold text-text-primary">Voice Generation Service</h3>
        ...
        <div className="p-6">
          <div className="p-4 bg-red-500/10 border border-red-500/30 rounded-lg">
            <p className="text-sm text-red-400">
              {status.error || 'Voice generation service is currently unavailable'}
            </p>
          </div>
        </div>
      ) : (
            ...
            <div className="p-6 border-b border-border-primary">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-base font-semibold text-text-primary">{subscription.tier}</p>
                </div>
                <div>
                  <p className="text-xs text-text-tertiary mb-1">Status</p>
                  <div className="flex items-center gap-2">
                    <CheckCircle className="w-4 h-4 text-green-400" />
                    <p className="text-base font-semibold text-green-400">{subscription.status}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-4">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <p className={cn('text-sm font-bold', getUsageColor(usagePercentage))}>
                    {usagePercentage}%
                  </p>
                </div>
                ...
                <div className="flex items-center justify-between mt-2">
                  <p className="text-xs text-text-secondary">
                    {voiceStatusClient.formatCharacterCount(subscription.character_count)} /{' '}
                    {voiceStatusClient.formatCharacterCount(subscription.character_limit)}
                  </p>
              ...
              <div className="mt-4 grid grid-cols-2 gap-3">
                <div className="p-3 bg-bg-tertiary rounded-lg">
                  ...
                  <p className="text-lg font-bold text-text-primary">{subscription.voice_limit}</p>
                </div>
                <div className="p-3 bg-bg-tertiary rounded-lg">
          ...
          <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            {rateLimit && (
              <div className="p-4 bg-gradient-to-br from-blue-500/10 to-blue-600/10 border border-blue-500/30 rounded-lg">
                ...
                {rateLimit.isLimited && (
                  <div className="mt-2 text-xs text-yellow-400">Rate limited</div>
                )}
              </div>
            )}
            ...
            <div className="p-4 bg-gradient-to-br from-purple-500/10 to-purple-600/10 border border-purple-500/30 rounded-lg">
              ...
              <p className="text-2xl font-bold text-text-primary">{voiceCount}</p>
            ...
            <div className="p-4 bg-gradient-to-br from-green-500/10 to-green-600/10 border border-green-500/30 rounded-lg">
              ...
              <p className="text-2xl font-bold text-text-primary">{modelCount}</p>
              <p className="text-xs text-text-tertiary mt-1">available models</p>
            </div>
          </div>
