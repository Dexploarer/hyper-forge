import { User as UserIcon, Settings, LogOut, ChevronDown, Mail, MessageSquare, CheckCircle } from 'lucide-react'
  ...
  return (
    <div className="relative" ref={menuRef}>
      ...
      >
        ...
        {!isProfileComplete && (
          <div className="w-2 h-2 bg-yellow-400 rounded-full animate-pulse" title="Profile incomplete" />
        )}
        ...
        <div className="absolute right-0 mt-2 w-80 bg-bg-primary border border-border-primary rounded-lg shadow-2xl z-50 overflow-hidden animate-fade-in">
          ...
          <div className="p-4 bg-gradient-to-br from-primary/10 to-primary/5 border-b border-border-primary">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                {isProfileComplete ? (
                  <Badge variant="success" className="mt-1 bg-green-500/20 text-green-400 border-green-500/30 text-xs">
                    <CheckCircle size={10} className="mr-1" />
                    Profile Complete
                  </Badge>
                ) : (
                  <Badge variant="secondary" className="mt-1 bg-yellow-500/20 text-yellow-400 border-yellow-500/30 text-xs">
                    Profile Incomplete
                  </Badge>
                )}
              </div>
          ...
          <div className="p-4 space-y-2 border-b border-border-primary">
            ...
            {!user.email && !user.discordUsername && (
              <p className="text-xs text-text-tertiary italic">No contact info</p>
            )}
          </div>
