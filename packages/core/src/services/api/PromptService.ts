...
export interface AssetTypePromptsByCategory {
  avatar: { default: Record<string, AssetTypePrompt>; custom: Record<string, AssetTypePrompt> }
  item: { default: Record<string, AssetTypePrompt>; custom: Record<string, AssetTypePrompt> }
}
...
class PromptServiceClass {
  ...
  constructor() {
    ...
    const apiBaseUrl = envApiUrl || (import.meta.env.PROD ? '/api' : 'http://localhost:3004/api')
  ...
  async deleteGameStyle(styleId: string): Promise<boolean> {
    try {
      const response = await apiFetch(`${this.baseUrl}/game-styles/${styleId}`, { method: 'DELETE', timeoutMs: 8000 })
      return response.ok
    } catch (error) {
  ...
  async getAssetTypePrompts(): Promise<AssetTypePromptsByCategory> {
    const response = await apiFetch(`${this.baseUrl}/asset-types`, { timeoutMs: 10000 })
  ...
  async deleteAssetType(typeId: string, category: 'avatar' | 'item'): Promise<boolean> {
    try {
      const response = await apiFetch(`${this.baseUrl}/asset-types/${typeId}?category=${category}`, { method: 'DELETE', timeoutMs: 8000 })
      return response.ok
    } catch (error) {
  ...
  async getMaterialPrompts(): Promise<MaterialPromptTemplate> {
    const response = await apiFetch(`${this.baseUrl}/materials`, { timeoutMs: 10000 })
    ...
    const templates = {
      runescape: data.templates?.runescape ?? '${materialId} texture, low-poly RuneScape style',
