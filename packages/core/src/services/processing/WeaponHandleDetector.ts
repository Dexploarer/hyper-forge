...
export class WeaponHandleDetector {
  ...
  async detectHandleArea(
    ...
    if (useConsensus) {
      ...
      const annotatedCanvas = this.drawGripArea(
        sideCanvas,
        gripData.gripBounds,
      );
  ...
  async exportNormalizedWeapon(
    ...
    // Note: File writing should be handled by the caller (server/scripts)
    // This method only returns the buffer for maximum flexibility
    if (outputPath) {
      console.log(
        `⚠️ outputPath parameter is deprecated. File writing should be handled by the caller.`,
      );
    }
  ...
  private async loadModel(modelUrl: string): Promise<THREE.Object3D> {
    ...
    try {
      ...
      if (!validationResponse.ok) {
        throw new Error(
          `Model endpoint returned ${validationResponse.status}: ${validationResponse.statusText}`,
        );
      }

      const contentType = validationResponse.headers.get("content-type");
      console.log("Model endpoint content-type:", contentType);

      // Check for binary GLTF content types
      ...
      if (!isValidContentType) {
        throw new Error(
          `Invalid content-type: ${contentType}. Expected binary GLTF (model/gltf-binary or application/octet-stream). ` +
            `The endpoint may be returning an error page instead of the model file.`,
        );
      }
      ...
      if (error instanceof Error) {
        if (error.message.includes("JSON")) {
          throw new Error(
            `Failed to load model: Received non-binary response (likely HTML/text error page). ` +
              `Check that the asset exists and the API endpoint is returning binary GLTF data. ` +
  ...
  private async getGripCoordinates(
    canvas: HTMLCanvasElement,
  ): Promise<GripCoordinates> {
  ...
  private async getConsensusGripCoordinates(
    ...
    for (const { angle, canvas } of canvases) {
      const promptIndex =
        canvases.indexOf(canvases.find((c) => c.angle === angle)!) %
        promptVariations.length;
        ...
        if (response.ok) {
          ...
          if (data.success && data.gripData) {
            ...
            const isValidDetection =
              bounds.minY > 200 && // Not in top 40% (blade area)
    ...
    const detectionsToUse =
      highConfidenceDetections.length >= 2
    ...
    const avgConfidence =
      detectionsToUse.reduce((sum, d) => sum + d.confidence, 0) / count;
    ...
    const weaponType = weaponTypes
      .sort(
        (a, b) =>
          weaponTypes.filter((t) => t === a).length -
          weaponTypes.filter((t) => t === b).length,
      )
      .pop();
  ...
  private backProjectTo3D(
    ...
    for (let i = 0; i <= sampleCount; i++) {
      for (let j = 0; j <= sampleCount; j++) {
        ...
        const screenX =
          redBoxBounds.minX + (redBoxBounds.maxX - redBoxBounds.minX) * uBiased;
        const screenY =
          redBoxBounds.minY + (redBoxBounds.maxY - redBoxBounds.minY) * vBiased;
    ...
    if (guardY === -1) {
      ...
      for (let y = canvas.height * 0.8; y > canvas.height * 0.2; y--) {
        if (widthProfile[y] > 0 && widthProfile[y - 5] > 0) {
          ...
          if (
            widthChange > maxWidthChange &&
            widthChange > widthProfile[y] * 0.5
          ) {
            maxWidthChange = widthChange;
            guardY = y - 5;
          }
  ...
  private async detectWeaponOrientation(
    canvas: HTMLCanvasElement,
  ): Promise<boolean> {
