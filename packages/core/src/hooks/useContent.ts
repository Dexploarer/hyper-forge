...
export const useContent = () => {
  ...
  const fetchAllContent = useCallback(async () => {
    try {
      ...
      const [npcsData, questsData, dialoguesData, loresData] = await Promise.all([
  ...
  const allContent: ContentItem[] = [
    ...npcs.map(npc => ({ id: npc.id, type: 'npc' as ContentType, name: npc.name, createdAt: new Date(npc.createdAt), data: npc })),
    ...quests.map(quest => ({ id: quest.id, type: 'quest' as ContentType, name: quest.title, createdAt: new Date(quest.createdAt), data: quest })),
    ...dialogues.map(dialogue => ({ id: dialogue.id, type: 'dialogue' as ContentType, name: dialogue.npcName, createdAt: new Date(dialogue.createdAt), data: dialogue })),
    ...lores.map(lore => ({ id: lore.id, type: 'lore' as ContentType, name: lore.title, createdAt: new Date(lore.createdAt), data: lore })),
  ]
