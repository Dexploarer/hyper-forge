...
export function useArmorFitting(): ArmorFittingState & ArmorFittingActions {
  ...
  const [visualizationMode, setVisualizationMode] = useState<
    "none" | "regions" | "collisions" | "weights" | "hull"
  >("none");
  ...
  const performFitting = useCallback(
    async (viewerRef: React.RefObject<ArmorFittingViewerRef>) => {
      ...
      try {
        ...
        console.log(
          "🎯 ArmorFittingLab: Phase 1 - Bounding box fit to position armor at torso",
        );
        ...
        if (fittingConfig.method === "hull") {
          ...
          console.log(
            "🎯 ArmorFittingLab: Starting hull-based fit with params:",
            hullParams,
          );
          ...
          fittingConfig.method === "collision" ||
          fittingConfig.method === "smooth"
        ) {
