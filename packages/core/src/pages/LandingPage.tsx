...
export function LandingPage() {
  ...
  const handleInputSubmit = async () => {
    if (inputValue.trim()) {
      // Authenticate - app will automatically show generation page after login
      const success = await login('admin123')
      if (!success) {
    ...
    <div className="min-h-screen bg-bg-primary flex flex-col relative overflow-hidden">
      ...
      <header className="relative z-20 w-full px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          />
          <span className="text-lg font-semibold text-text-primary">Asset Forge</span>
        </div>
        ...
        <nav className="flex items-center gap-6">
          <a 
            ...
            onClick={(e) => {
              e.preventDefault()
              // TODO: Link to docs when available
            }}
          >
            Docs
          </a>
