...
import { contentGenerationRoutes } from "../../../../server/routes/content-generation";
...
describe("Content Generation Routes", () => {
  ...
  beforeAll(() => {
    // Create test app with mocked content generation routes
    app = new Elysia().use(contentGenerationRoutes);
  });

  describe("GET /api/content/test", () => {
    it("should return test message", async () => {
      const response = await app.handle(
        new Request("http://localhost/api/content/test"),
      );

      expect(response.status).toBe(200);
      const data = await response.json();
      expect(data.message).toBe("Content generation routes are working!");
    });
  });
